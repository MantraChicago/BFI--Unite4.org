...
var FilterView=Backbone.View.extend({
  ...
  this.render()
    ...
    render:function(){
      var template=$( JST['templates/map/filter_template']())

      for(var name in this.options.filterOptions){
          var models=this.options.filterOptions[name].models
          var html=$( JST['templates/map/filter_header_row_template']({ name:name}))
          for(var i in models){
          var model = models[i]
          html.find('.filterHolder').append(JST['templates/map/filter_row_template']({
              type:name,
              id:model.id,
              name:model.get('name')
          }))
